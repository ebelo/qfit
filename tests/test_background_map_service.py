...
class ApplyLabelPriorityRealTests(unittest.TestCase):
    ...
    def test_light_major_rank_boundaries_follow_source_with_existing_city_guard(self):
        import json
        import math
        from pathlib import Path
        ...
        from qfit.mapbox_config import simplify_mapbox_style_expressions
        from qfit.validation.mapbox_outdoors_path_pedestrian_focus import _mapbox_filter_value

        source = json.loads((Path(__file__).parent / "fixtures/mapbox/light-place-name-source.json").read_text())
        owner = next(x for x in source["layers"] if x["id"] == "settlement-major-label")
        layer = MagicMock()
        self.service._apply_mapbox_gl_style(
            layer, simplify_mapbox_style_expressions(source), source_style_definition=source,
        )
        ...
        cases = [{"symbolrank": rank, "filterrank": 2, "class": cls, "worldview": worldview, "type": "city"}
                 for rank in range(21)
                 for cls in ("settlement", "disputed_settlement")
                 for worldview in ("all", "US")]
        valid = {"symbolrank": 6, "filterrank": 2, "class": "settlement", "worldview": "all", "type": "city"}
            ...
            for properties in cases:
                with self.subTest(zoom=zoom, properties=properties):
                    ...
                    for rule in active:
                        ...
                        self.assertFalse(expression.hasEvalError(), expression.evalErrorString())
                        observed = value != NULL and bool(value)
                        self.assertEqual(rule.labelSettings().fieldName, 'coalesce("name_en", "name")')
