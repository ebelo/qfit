...
Run after completing the 14-camera matrix. Input files are never regenerated here.
...
cameras=load(P/'cameras.json');assert len(cameras)==14
pairs=[];anchors=[];metrics=[];placements=[];settings=[]
natural_names=[n+s for n in ('natural-line-label','natural-point-label') for s in ('','-qfit-open-fonts-z8-plus')]
...
for v in ('3','4'):
 ...
 for row in normalized:
  if row['style_name'] in natural_names:
 ...
 providers={str(i) for i,row in enumerate(after) if row['style_name'] in natural_names};assert providers=={'5','6','7','8'}
 for camera in cameras:
  ...
  placements.append({'camera':camera,'runtime':v,'before_naturals':targets[0],'after_naturals':targets[1],
   'before_natural_texts':sorted({r['text'] for r in targets[0]}),'after_natural_texts':sorted({r['text'] for r in targets[1]}),
   'nonnatural_text_population_equal':counts[0]==counts[1],'nonnatural_text_and_corners_equal':exact(other[0])==exact(other[1]),
   'removed_nonnatural_records':[{'key':k,'count':n} for k,n in (counts[0]-counts[1]).items()],
   'added_nonnatural_records':[{'key':k,'count':n} for k,n in (counts[1]-counts[0]).items()],
  ...
  if camera=='cairo-nile-z14':crops['crop']=(270,30,650,330)
  elif camera=='mont-blanc-z10':crops['crop']=(0,600,380,900)
  elif camera=='geneva-urban-z14-light':crops['crop']=(140,380,520,680)
  for kind,box in crops.items():
   ...
   for i,(path,label) in enumerate([(P/'reference-mercator'/camera/'mapbox.png','Mapbox Mercator reference'),(P/v/camera/'before/qgis.png',f'QGIS{v} Before 3436e38'),(P/v/camera/'after/qgis.png',f'QGIS{v} After cbf2320')]):
...
for row in placements:
 if not row['nonnatural_text_population_equal']:print('nonnatural_population_changed',row['runtime'],row['camera'],row['removed_nonnatural_records'],row['added_nonnatural_records'])
