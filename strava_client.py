...
class StravaClient:
    ...
    def fetch_activities(self, per_page=50, max_pages=1, before=None, after=None):
        ...
        for page in range(1, max_pages + 1):
            ...
            url = f"{self.ACTIVITIES_URL}?{urlencode(params)}"
    ...
    def _request_json(self, request):
        try:
            ...
            raise StravaClientError(f"Strava API error {exc.code}: {body}") from exc
        except URLError as exc:
            raise StravaClientError(f"Network error talking to Strava: {exc}") from exc
        except json.JSONDecodeError as exc:
            raise StravaClientError(f"Invalid JSON returned by Strava: {exc}") from exc
