...
def _sum_numeric_rows(rows: list[dict], field_name: str, caster) -> int | float:
    return sum(
        value
        for value in (caster(row.get(field_name)) for row in rows)
        if value is not None
    )


def _build_cover_extent(rows: list[dict]) -> dict:
...
def _collect_atlas_activity_ids(rows: list[dict]) -> list[str]:
    ...
    return atlas_activity_ids


def _build_cover_labels(rows: list[dict]) -> dict:
    activity_count = len(rows)
    page_dates = [str(value) for value in (row.get("page_date") for row in rows) if value]
    total_distance_m = _sum_numeric_rows(rows, "distance_m", _safe_float)
    total_moving_time_s = _sum_numeric_rows(rows, "moving_time_s", _safe_int)
    total_elevation_gain_m = _sum_numeric_rows(rows, "total_elevation_gain_m", _safe_float)
...
def build_cover_summary_from_rows(rows: Iterable[dict]) -> dict:
    ...
    return {
        **_build_cover_labels(rows),
        **_build_cover_extent(rows),
        "_atlas_activity_ids": _collect_atlas_activity_ids(rows),
    }
