"""Replay fixed Light airport-content captures from the archived source snapshot.

Use baseline ce90038 or candidate 547cc22 as --repo, with recorded font-enabled
...
p.add_argument('--variant',choices=['before-final','before-repeat','after-final','after-repeat','before-pdf','after-pdf','after-replay'],default='before-final')
...
for name,lon,lat,zoom in (
    ('cairo-airport-z8',31.405,30.111,8),
    ('geneva-airport-z10',6.109,46.238,10),
    ('geneva-airport-z14',6.109,46.238,14),
    ('cairo-airport-z14',31.405,30.111,14)):
...
def worker(camera_name, variant, version):
    ...
    if not a.pdf:qgis.core.QgsMapRendererParallelJob=ObservedJob
