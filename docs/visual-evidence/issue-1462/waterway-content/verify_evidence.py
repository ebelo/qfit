...
Run after completing the 13-camera matrix. Input files are never regenerated here.
...
cameras=load(P/'cameras.json');assert len(cameras)==13
pairs=[];anchors=[];metrics=[];placements=[];settings=[]
waterway_names=['waterway-label-z13-to-z15','waterway-label-z15-to-z17','waterway-label-z17-plus']
...
for v in ('3','4'):
 ...
 for row in normalized:
  if row['style_name'] in waterway_names:
 ...
 providers={str(i) for i,row in enumerate(after) if row['style_name'] in waterway_names};assert providers=={'2','3','4'}
 for camera in cameras:
  ...
  placements.append({'camera':camera,'runtime':v,'before_waterways':targets[0],'after_waterways':targets[1],
   'before_waterway_texts':sorted({r['text'] for r in targets[0]}),'after_waterway_texts':sorted({r['text'] for r in targets[1]}),
   'nonwaterway_text_population_equal':counts[0]==counts[1],'nonwaterway_text_and_corners_equal':exact(other[0])==exact(other[1]),
   'removed_nonwaterway_records':[{'key':k,'count':n} for k,n in (counts[0]-counts[1]).items()],
   'added_nonwaterway_records':[{'key':k,'count':n} for k,n in (counts[1]-counts[0]).items()],
  ...
  if camera.startswith('geneva-confluence'):crops['crop']=(380,360,760,660)
  ...
  for kind,box in crops.items():
   ...
   for i,(path,label) in enumerate([(P/'reference-mercator'/camera/'mapbox.png','Mapbox Mercator reference'),(P/v/camera/'before/qgis.png',f'QGIS{v} Before 78b183b'),(P/v/camera/'after/qgis.png',f'QGIS{v} After 09c8715')]):
...
for row in placements:
 if not row['nonwaterway_text_population_equal']:print('nonwaterway_population_changed',row['runtime'],row['camera'],row['removed_nonwaterway_records'],row['added_nonwaterway_records'])
