...
For filter-specific converter triage, add `--include-qgis-filter-parse-support`. This implies `--include-qgis-converter-warnings` and isolates each remaining qfit-preprocessed filter expression in a minimal same-type QGIS converter style. The resulting report counts how many filters the QGIS expression parser accepts, groups parser-rejected filters by visual layer group and operator signature, and lists compact filter excerpts for the unsupported layers. Treat it as parser attribution only: a parser-accepted filter can still need visual validation, while a rejected filter identifies a stronger candidate for deeper simplification research.
