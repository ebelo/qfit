...
def api():
    ...
    with patch.dict('sys.modules', {'qgis.core': core, 'qgis.PyQt.QtGui': gui,
                                   'qgis.PyQt.QtCore': MagicMock()}):
        yield SimpleNamespace(core=core, gui=gui, image=image)
    collision._svg_wrapped_sprite.cache_clear()


def test_wrapper_preserves_original_sprite_bytes(api):
...
@pytest.mark.parametrize('value', ['external-sprite.png', 'base64:invalid!'])
def test_rejects_non_inline_or_invalid_sprite(api, value):
    with pytest.raises(ValueError):
        collision._svg_wrapped_sprite(value)


def test_rejects_non_png(api):
...
def test_static_and_feature_selected_sprites_couple_without_deleting_styles(api):
...
@pytest.mark.parametrize('case', ['other-style', 'high-zoom', 'no-owner', 'disabled', 'line',
                                 'filter', 'no-symbol', 'multilayer', 'non-raster', 'background',
                                 'external-path', 'unsupported-selector'])
def test_unsupported_pairs_leave_existing_rendering_intact(api, case):
    label, owner = pair()
