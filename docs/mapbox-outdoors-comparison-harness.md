...
For Mapbox Light, select the Light defaults and use the token-file option so the credential is read inside Python rather than exposed through the process arguments:

```bash
python3 validation/mapbox_outdoors_source_crop_overlap.py \
  --preset light \
  --visual-crop-json debug/mapbox-light-visual-crops/<timestamp>/visual-crops.json \
  --camera bern-urban-z12-light \
  --mapbox-token-file /path/to/mapbox-token.txt
```
...
Comparison runs fetch one source style for both renderers, persist a token-redacted `mapbox-source-style.json`, and record its SHA-256 fingerprint in manifests and style-adjustment reports. The preprocessed QGIS style retains a separate fingerprint. This distinguishes live Mapbox style revisions even when qfit preprocessing discards the changed source property, while also detecting qfit preprocessing changes against an unchanged source. Aggregation rejects reports when either fingerprint differs; regenerate the full camera matrix from one source and qfit preprocessing revision instead of combining stale and current evidence.
